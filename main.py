...
def getModelProps(modelName):
    df = pandas.read_csv('output/models.csv', sep=",", index_col=0)
    data = df.values
    modelNames = data[:, 0]
...
def play_as_ai():
    ...
    def train(model):
        ...
        while not gameQuit:
            ...
            s_t1 = image_t1.reshape(1, int(image_t1.shape[0]), int(image_t1.shape[1] / INPUT_STACK), INPUT_STACK) ## 1x(HEIGHT * 0.1)x(WIDTH * 0.1)xINPUT_STACK
